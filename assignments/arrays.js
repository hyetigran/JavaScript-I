...
let carModels = inventory.map(state => {
...
let carYears = inventory.map(state => {
	return state.car_year;
});
...
let oldCars = inventory.filter(state => {
...
let BMW = inventory.filter(state => {
	return state.car_make === 'BMW';
});
let Audi = inventory.filter(state => {
	return state.car_make === 'Audi';
});
